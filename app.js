...
const IGNORED_STORAGE_KEY="holidayapp_ignored_v1";
  ...
  try{
    ...
    if(saved&&typeof saved==="object"){
      return{
        ...
        showIgnored:saved.showIgnored===true
      };
    }
  }catch{}
  return{
    types:new Set(ALL_TYPES),
    areas:new Set(ALL_AREAS),
...
function loadIgnoredItems(){
  try{
    const saved=JSON.parse(localStorage.getItem(IGNORED_STORAGE_KEY)||"[]");
    return new Set(Array.isArray(saved)?saved.filter(v=>typeof v==="string"):[]);
  }catch{return new Set();}
}

const restoredFilters=loadPersistentFilters();
const ignoredKeys=loadIgnoredItems();
const state={
  types:restoredFilters.types,
  areas:restoredFilters.areas,
  sources:restoredFilters.sources,
  showIgnored:restoredFilters.showIgnored,
  search:"",
  view:"list",
  gps:null
};
...
function saveIgnoredItems(){
  try{localStorage.setItem(IGNORED_STORAGE_KEY,JSON.stringify([...ignoredKeys]));}catch{}
}
...
function retryFailedCoordsOnce(){
  ...
  let changed=false;
  for(const [k,v] of Object.entries(coordCache)){
    if(v?.failed){delete coordCache[k];changed=true;}
  }
...
function isIgnored(i){return ignoredKeys.has(typeof i==="string"?i:itemKey(i))}

function filteredItems(){
  const q=state.search.trim().toLowerCase();
  return ITEMS.filter(i=>
    state.types.has(i.type)&&
    state.areas.has(i.area)&&
    state.sources.has(sourceLabel(i))&&
    (state.showIgnored||!isIgnored(i))&&
    (!q||[i.name,i.location,i.area,i.type,i.description].join(" ").toLowerCase().includes(q))
  );
}
...
function itineraryEntries(){
  try{
    const d=JSON.parse(localStorage.getItem(ITINERARY_STORAGE_KEY)||"{}");
...
function itineraryFlags(i){
  const k=itemKey(i);
  return itineraryEntries()
    .filter(x=>x.itemKey===k)
...
function km(a,b){
  const R=6371,r=x=>x*Math.PI/180,d1=r(b.lat-a.lat),d2=r(b.lng-a.lng);
...
function flagsHtml(i){
  ...
  const ignored=isIgnored(i)?'<span class="card-flag" style="background:#eceeed;color:#59615d">Ignored</span>':"";
  const planned=itineraryFlags(i)
    .map(x=>`<span class="card-flag flag-itinerary">✓ ${esc(fmtDay(x.date))} · ${esc(x.section)}</span>`)
    .join("");
  return`<div class="card-flags"><span class="card-flag ${sourceClass}">${esc(source)}</span>${ignored}${planned}</div>`;
}

function render(){
  const r=filteredItems();
  if(E.count){
    const hiddenIgnored=!state.showIgnored?ignoredKeys.size:0;
    E.count.textContent=`${r.length} of ${ITEMS.length} places${hiddenIgnored?` · ${hiddenIgnored} ignored`:""}`;
  }
  if(!E.cards)return;

  E.cards.innerHTML=r.length?r.map(i=>{
    const d=distanceText(i),k=itemKey(i),ignored=isIgnored(i);
    return`<article class="card${ignored?" ignored-card":""}" data-type="${esc(i.type)}" data-item-key="${esc(k)}"><div class="accent"></div><div class="card-body">
      ...
      <div class="actions">
        ...
        <button class="btn ignore-item-btn" type="button" data-ignore-item="${esc(k)}">${ignored?"Unignore":"Ignore"}</button>
      </div></div></article>`;
...
function areaHint(area){
  return({
    "Dublin":"Dublin",
    "Galway":"Galway",
    "Roscommon / Athlone":"Athlone Roscommon",
    "Limerick / Clare":"Limerick Clare",
    "Killarney / Kerry":"Killarney Kerry"
  })[area]||area||"Ireland";
}
function sleep(ms){return new Promise(r=>setTimeout(r,ms))}
function nominatimRequest(params){
  const url="https://nominatim.openstreetmap.org/search?format=jsonv2&addressdetails=1&limit="+
    encodeURIComponent(params.limit||1)+"&countrycodes=ie&q="+encodeURIComponent(params.q);
...
async function tryGeocode(item,query){
  try{
    ...
    if(data?.[0]){
      ...
      saveCoordCache();
      document.dispatchEvent(new CustomEvent("holidayapp:index-place",{detail:{item,success:true}}));
      return true;
    }
...
function emitIndexProgress(done,total,failed=0,finished=false){
  document.dispatchEvent(new CustomEvent("holidayapp:index-progress",{
    detail:{done,total,failed,finished,located:ITEMS.filter(hasCoords).length}
  }));
}
async function indexAllPlacesOnce(){
  ...
  placeIndexPromise=(async()=>{
    ...
    let done=total-unresolved.length;
    emitIndexProgress(done,total,0,false);
    ...
    emitIndexProgress(total,total,finalFailures.length,true);
...
function ensureIgnoreFilterButton(){
  if(!E.sources||E.sources.querySelector("[data-show-ignored]"))return;
  const b=document.createElement("button");
  b.className="pill";
  b.type="button";
  b.dataset.showIgnored="true";
  E.sources.appendChild(b);
}
...
function updateFilterButtons(){
  if(E.types){
    E.types.querySelectorAll("[data-type]").forEach(b=>{
      ...
      b.classList.toggle("active",v==="All"?state.types.size===ALL_TYPES.length:state.types.has(v));
    });
  }
  if(E.areas){
    E.areas.querySelectorAll("[data-area]").forEach(b=>{
      const v=b.dataset.area;
      b.classList.toggle("active",v==="All"?state.areas.size===ALL_AREAS.length:state.areas.has(v));
    });
  }
  if(E.sources){
    E.sources.querySelectorAll("[data-source]").forEach(b=>{
      const v=b.dataset.source;
      b.classList.toggle("active",v==="All"?state.sources.size===ALL_SOURCES.length:state.sources.has(v));
    });
    const ignoredButton=E.sources.querySelector("[data-show-ignored]");
    if(ignoredButton){
      ignoredButton.classList.toggle("active",state.showIgnored);
      ignoredButton.textContent=`Show Ignored${ignoredKeys.size?` (${ignoredKeys.size})`:""}`;
      ignoredButton.setAttribute("aria-pressed",state.showIgnored?"true":"false");
    }
  }
}
...
function emitFiltersChanged(){
  document.dispatchEvent(new CustomEvent("holidayapp:filters-changed",{
    detail:{
      types:[...state.types],
      areas:[...state.areas],
      sources:[...state.sources],
...
function toggleIgnoredByKey(key){
  if(!key)return;
...
function useGps(){
  if(!navigator.geolocation){
    E.gpsStatus.textContent="GPS not supported by this browser.";
    return;
  }
  E.gpsBtn.disabled=true;
  E.gpsStatus.textContent="Requesting location permission…";
...
E.types.addEventListener("click",e=>{
  ...
  if(!b)return;
  toggleSet(state.types,b.dataset.type,ALL_TYPES);
  filtersChanged();
});
E.areas.addEventListener("click",e=>{
  const b=e.target.closest("[data-area]");
  if(!b)return;
  toggleSet(state.areas,b.dataset.area,ALL_AREAS);
  filtersChanged();
});
E.sources?.addEventListener("click",e=>{
  const ignoreToggle=e.target.closest("[data-show-ignored]");
  if(ignoreToggle){
  ...
  if(!b)return;
  toggleSet(state.sources,b.dataset.source,ALL_SOURCES);
  filtersChanged();
});
E.cards?.addEventListener("click",e=>{
  const b=e.target.closest("[data-ignore-item]");
  if(!b)return;
  e.preventDefault();
  e.stopPropagation();
  toggleIgnoredByKey(b.dataset.ignoreItem);
});
